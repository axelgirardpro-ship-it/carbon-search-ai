...
const searchClient = {
  ...
  search: (requests: any[]) => {
    ...
    const cleanedRequests = requests.map((request, index) => {
      ...
      // Préserver tous les numericFilters, y compris ceux concernant FE (issus des widgets)
      // afin que les raffinements numériques (min/max) s'appliquent côté Algolia avant la pagination.
